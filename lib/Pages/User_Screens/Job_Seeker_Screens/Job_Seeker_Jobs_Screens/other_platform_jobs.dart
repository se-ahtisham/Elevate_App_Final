...
class OtherPlatformJobsState extends ConsumerState<OtherPlatformJobs> {
  ...
  Future<void> loadPassedSkillsAndJobs() async {
    ...
    try {
      ...
      if (myID != null) {
        final bestScores = await firebaseService.getBestPassedScoresBySkill(
          myID,
        );
      ...
      if (skillNames.isNotEmpty) {
        ...
        final bronzePool = allFetched
            .where((j) => _apiJobTier(j) == 'Bronze')
            .toList();
        final silverPool = allFetched
            .where((j) => _apiJobTier(j) == 'Silver')
            .toList();
        final goldPool = allFetched
            .where((j) => _apiJobTier(j) == 'Gold')
            .toList();
